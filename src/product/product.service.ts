...
export class ProductService {
  ...
  async paginate(options: IPaginationOptions): Promise<Pagination<Product>> {
    const query = this.productRepository
      ...
      //.where('product.isActive = :isActive', { isActive: true }) discontinued
  ...
  async updateStatus(id: string, updateStatusProductdto: UpdateStatusProductDto) {
    const product = await this.findById(id);

    const status = await this.statusService.findOne(updateStatusProductdto.status as StatusType);
